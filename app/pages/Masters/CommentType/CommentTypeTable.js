...
import CommentTypeSearchForm from './CommentTypeSearchForm';
...
class CommentTypeTable extends Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedRowKeys: [],
  ...
  tableColumns = [
    {
      ...
      onFilter: (value, record) => {
        const returnValue =
          (record.typeName ? record.typeName.toLowerCase() : '') +
        ...
        return searchText ? (
          searchText.match(numbers) ? (
            ...
            <div>
              ...
              <span className={styles.anchorCode}>({data.id})</span>
            </div>
          )
        ) : (
          <div>
  ...
  tableHeader = () => (
    <div>
      <TableOptions
        ...
        searchForm={CommentTypeSearchForm}
      />
    </div>
  );
  ...
  toggleTableRowsSelected = value => {
    ...
    if (value === true) {
      this.setState(
        ...
        () => {
          this.state.selectedObjects.map(v => (v.dialogContent = v.typeName));
          const dataForAction = {
...
export default connect(({ commentType }) => ({
  data: commentType.reducerList.data || [],
}))(CommentTypeTable);
