...
class CommentTypeForm extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: newObject,
  ...
  checkIsExists = event => {
    ...
    const isSameName = _.isEqual(dataForExists, this.state.data.typeName);
    if (
      !isSameName &&
  ...
  enableSaveButton = (enableFlag, isExistMessageFlag) => {
    ...
    valuesObject.typeName = valuesObject.typeName.trim();
      ...
      <div className={styles.dataViewMode1}>
        <div className={styles.tabPaneCard2}>
          ...
          <div className={styles.viewLabelContainer}>
            ...
            <div className={styles.viewLabelSeparator}>:</div>
            <div className={styles.viewText}>{data.typeName}</div>
          </div>
          <div className={styles.viewLabelContainer}>
...
export default Form.create()(
  connect(({ commentType }) => ({
    dataById: commentType.reducerById,
  }))(CommentTypeForm),
);
