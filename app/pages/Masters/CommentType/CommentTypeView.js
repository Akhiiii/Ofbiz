...
import CommentTypeTable from './CommentTypeTable';
...
class CommentTypeView extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
  ...
  handleSubmitAction = (actionType, dataForActionParam, nameSpaceParam) => {
    ...
    if (actionType === actionConsts.ACTION_TYPE_MULTIPLE_SELECTION) {
      ...
      dataForAction.dialogContent = dataForAction.typeName;
      pageTitleObject.push(dataForAction);
      const selectedName = {
        pageTitle: dataForAction.typeName,
        pageSubTitle: dataForAction.id,
...
export default Form.create()(
  connect(({ commentType }) => ({ commentType }))(CommentTypeView),
);
