...
import { NAMESPACE } from './CityConsts';

const pageURL = `${baseURL}/${NAMESPACE}`;

const saveURL = `${pageURL}/save`;
const saveallURL = `${pageURL}/saveall`;
...
const statesByCountryListURL = `${baseURL}/${'state'}/statesByCountryList`;
const byIdURL = `${pageURL}/id`;
const existsURL = `${pageURL}/isexists`;

export const serviceSave = values => axios.post(`${saveURL}`, values);

export const serviceSaveall = values => axios.post(`${saveallURL}`, values);

export const serviceDelete = id => axios.delete(`${deleteURL}?id=${id}`);

export const serviceDeleteAll = ids => axios.delete(`${deleteAllURL}?ids=${ids}`);
...
export const serviceUnblockAll = ids => axios.get(`${unblockAllURL}?ids=${ids}`);

export const serviceActivate = id => axios.get(`${activateURL}?id=${id}`);

export const servicestatesByCountryList = id => axios.get(`${statesByCountryListURL}?id=${id}`);

export const serviceActivateAll = ids => axios.get(`${activateAllURL}?ids=${ids}`);

export const serviceDeactivate = id => axios.get(`${deactivateURL}?id=${id}`);

export const serviceDeactivateAll = ids => axios.get(`${deactivateAllURL}?ids=${ids}`);
