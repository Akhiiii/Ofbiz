...
import PriorityLevelSearchForm from './PriorityLevelSearchForm';
...
class PriorityLevelTable extends Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedRowKeys: [],
  ...
  tableHeader = () => (
    <div>
      <TableOptions
        ...
        searchForm={PriorityLevelSearchForm}
      />
    </div>
  );
  ...
  toggleTableRowsSelected = value => {
    ...
    if (value === true) {
      this.setState(
        ...
        () => {
          this.state.selectedObjects.map(v => (v.dialogContent = v.level));
          const dataForAction = {
...
export default connect(({ priorityLevel }) => ({
  data: priorityLevel.reducerList.data || [],
}))(PriorityLevelTable);
