...
class PriorityLevelView extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
  ...
  handleSubmitAction = (actionType, dataForActionParam, nameSpaceParam) => {
    ...
    if (actionType === actionConsts.ACTION_TYPE_MULTIPLE_SELECTION) {
      ...
      dataForAction.dialogContent = dataForAction.level;
      pageTitleObject.push(dataForAction);
      const selectedName = {
        pageTitle: dataForAction.level,
...
export default Form.create()(
  connect(({ priorityLevel }) => ({ priorityLevel }))(PriorityLevelView),
);
